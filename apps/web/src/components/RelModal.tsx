...
export function RelModal() {
  ...
  const cands = allTasks()
    .filter(
      (x) =>
        ...
        (x.t + " ATL-" + x.id).toLowerCase().includes(q),
    )
    .slice(0, 40);

  const link = (otherId: number) => {
    addRelationship(t.id, type, otherId);
    toast(`Linked ATL-${otherId}`);
    close();
  };
  ...
  return (
    <div className="modal rel-modal show" onClick={(e) => e.target === e.currentTarget && close()}>
      <div className="mh">
        <h3>Link issue</h3>
        <span className="status neutral"><span className="d" />ATL-{t.id}</span>
          ...
          {cands.length ? (
            cands.map((x) => (
              <button key={x.id} className="rp-item" onClick={() => link(x.id)}>
                ...
                <span className="rp-key mono">ATL-{x.id}</span>
