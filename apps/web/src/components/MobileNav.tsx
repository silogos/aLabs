...
export function MobileNav() {
  ...
  return (
    <aside className={`m-sheet ${mNavOpen ? "show" : ""}`} data-od-id="mobile-nav" aria-hidden={!mNavOpen}>
      ...
      <div className="m-body">
        <button className={`m-acct ${sheetView === "acct" ? "open" : ""}`} onClick={() => setSheetView(sheetView === "acct" ? "menu" : "acct")}>
          <span className="av">{name.split(" ").map((w) => w[0]).join("").slice(0, 2)}</span>
          <span className="tx">
            <b>{name}</b>
            <small>
