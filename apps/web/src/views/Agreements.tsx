      ...
      <div className="agr-grid">
        <div className="card">
          ...
          <div className="panel-body flush">
            {isLoading && <div className="tiny faint" style={{ padding: 20, textAlign: "center" }}>Loading…</div>}
            {!isLoading && list.length === 0 && <div className="tiny faint" style={{ padding: 20, textAlign: "center" }}>No agreements in this view</div>}
            {list.map((x) => {
              const xty = AGR_TYPE[x.type ?? "other"];
              const xst = AGR_ST[x.status];
              const [xbg, xfg] = toneVars(xty.tone);
              return (
                <div key={x.id} className={`agr-item ${x.id === a?.id ? "sel" : ""}`} onClick={() => setCurrentId(x.id)}>
                  ...
                  <div className="ai-foot">
                    <span className="small muted truncate" style={{ maxWidth: 150 }}>{x.counterparty}</span>
        ...
        {!a ? (
          <div className="card agr-detail tiny faint">No agreements yet — use “New agreement” to create the first one.</div>
        ) : (
          <AgreementDetail
            a={a}
            pid={pid}
...
function AgreementDetail({
  ...
  const prog: Record<AgreementStatus, number> = { draft: 0, sent: 1, accepted: 2, rejected: 2, expired: 2 };
  const finalLbl =
    a.status === "accepted" ? "Accepted & counter-signed" : a.status === "rejected" ? "Rejected by client" : a.status === "expired" ? "Agreement expired" : "Counter-signed";
  const owner = personOf(a.owner?.id);
  const ownerName = owner?.name ?? "Draft owner";

  const steps = [
    { t: "Draft created", d: `${ownerName} prepared the ${ty.l}${a.startDate ? " · effective " + fmtD(a.startDate) : ""}`, cls: prog[a.status] >= 0 ? "done" : "future", cur: a.status === "draft" },
    { t: `Sent to ${a.counterparty}`, d: a.sentAt ? "Sent " + fmtD(a.sentAt) : "Not yet sent", cls: a.sentAt || prog[a.status] >= 1 ? "done" : "future", cur: a.status === "sent" },
    {
      t: finalLbl,
      d:
        a.status === "accepted" ? (a.signedAt ? "Counter-signed " + fmtD(a.signedAt) : "Awaiting counter-signature")
          : a.status === "rejected" ? "Declined " + (a.sentAt ? fmtD(a.sentAt) : "")
            : a.status === "expired" ? "Term ended " + (a.endDate ? fmtD(a.endDate) : "")
              : "Awaiting outcome",
  ...
  return (
    <div className="card agr-detail">
      <div className="agr-head">
        ...
        {a.status === "draft" && <div className="agr-actions"><button className="btn primary sm" onClick={() => onStatus(a.id, "sent")}>Send to client</button></div>}
        {a.status === "sent" && (
          <div className="agr-actions">
            <button className="btn primary sm" onClick={() => onStatus(a.id, "accepted")}>Mark accepted</button>
            <button className="btn subtle sm" onClick={() => onStatus(a.id, "rejected")}>Decline</button>
          </div>
        )}
      </div>

      <div className="agr-facts">
        <div className="fact"><div className="fl">Contract value</div><div className="fv">{money(a.value, a.currency)}</div></div>
        <div className="fact"><div className="fl">Effective</div><div className="fv">{fmtD(a.startDate)}</div></div>
        <div className="fact"><div className="fl">Expires</div><div className="fv">{fmtD(a.endDate)}</div></div>
        <div className="fact"><div className="fl">Counter-signed</div><div className="fv">{fmtD(a.signedAt)}</div></div>
      </div>
      ...
      <div className="agr-parties">
        ...
        <div className="agr-party">
          <span className="av b" style={{ width: 34, height: 34, fontSize: 12 }}>{a.counterparty.split(" ").map((w) => w[0]).slice(0, 2).join("")}</span>
          <div><div className="role">Client</div><div className="nm">{a.counterparty}</div></div>
        </div>
        <div className="agr-party">
          {a.owner ? <AvKey id={a.owner.id} size="" /> : <span className="av c" style={{ width: 34, height: 34, fontSize: 12 }}>—</span>}
          <div><div className="role">Internal owner</div><div className="nm">{a.owner?.name ?? "Unassigned"}</div></div>
        </div>
      </div>
  ...
  return (
    <div className="modal show" onClick={(e) => e.target === e.currentTarget && onClose()}>
      ...
      <div className="mb">
        ...
        <input className={`fld ${err ? "err" : ""}`} value={fTitle} onChange={(e) => { setFTitle(e.target.value); setErr(false); }} placeholder={AGR_TP_HINT[fTy].ph} style={{ marginBottom: 12 }} />
        {err && <div className="fld-err show" style={{ marginBottom: 12 }}>Please enter a title.</div>}
      ...
      <div className="mf">
        ...
        <button className="btn primary" disabled={busy} onClick={() => void createDraft()}>Create draft</button>
      </div>
    </div>
  );
}
