          ...
          <div className="cols-wrap">
            ...
            >
              ...
              >
                ...
                <rect x="11" y="6" width="11" height="12" rx="2" />
              </svg>
              Sprint{" "}
          ...
          <>
            ...
            <label className="sel-wrap">
              ...
              <select
                className="sel"
                value={density}
                onChange={(e) => setDensity(e.target.value as "comf" | "comp")}
              >
            ...
            <div className="cols-wrap">
              ...
              {colsPop && (
                <div ref={colsPopState.popRef} className="cols-pop" style={colsPopState.style}>
                  ...
                  {["type", "key", "status", "priority", "assignee", "sprint", "points", "due"].map(
                    (c) => (
      ...
      {selected.size > 0 && (
        <div className="bulk-bar show">
          ...
          <select
            ...
            onChange={(e) => {
              if (e.target.value) {
                bulkSetAssignee([...selected], e.target.value);
                setSelected(new Set());
              }
            }}
          >
          ...
          return (
            <div className="col" key={c.id}>
              ...
              >
                ...
                {items.length === 0 && (
                  <div className="tiny faint empty-col">
                    No issues here · <span className="link">add one</span>
                  </div>
                )}
              </div>
            </div>
    ...
    >
      ...
      {t.epic && (
        <div className="epic-line">
          <EpicChip epic={t.epic} />
        </div>
      )}
      {(t.lb || []).length > 0 && (
        <div className="trow">
          {(t.lb || []).slice(0, 3).map((l) => (
      ...
      <div className="tmeta">
        ...
        <PtsPill pts={t.pts} />
        <span className={`due ${late(t) ? "late" : ""}`}>
          <svg
            width="11"
            height="11"
            viewBox="0 0 24 24"
...
function TableView(props: {
        ...
        : sortBy === "summary"
          ...
          : sortBy === "points"
            ...
            : sortBy === "due"
              ? t.due || ""
              : sortBy === "priority"
                ? ["p1", "p2", "p3", "p4"].indexOf(t.p)
                : (t as unknown as Record<string, string>)[sortBy] || "";
  ...
  const cellContent = (t: TaskRow, k: string) => {
    switch (k) {
      ...
      case "key":
        return <span className="mono id">{taskSerial(t.id)}</span>;
      case "status":
        return <StatusBadge s={t.s} />;
      case "priority":
        return <PrioBadge p={t.p} />;
      case "assignee":
        return (
      ...
      case "points":
        return t.pts ? <span className="mono">{t.pts}</span> : <span className="faint">—</span>;
      case "due":
        return <span className={`mono ${late(t) ? "due-over" : "muted"}`}>{t.due}</span>;
      default:
        return null;
    }
  };
  ...
  if (filtered.length === 0) {
    return (
      <div className="empty">
        ...
        <b>No issues match your view</b>
        <p>Try clearing the search or switching to the Tree.</p>
      </div>
    );
  }
              ...
              <tr>
                ...
                {vis.map((c) => (
                  ...
                  >
                    ...
                    {sortBy === c.k && (
                      <span className={`srt-arr ${sortDir < 0 ? "d" : ""}`}>▲</span>
                    )}
                  </th>
                ))}
                <th className="ck-col"></th>
                      ...
                      <tr className="grp-row">
                        ...
                        <td colSpan={vis.length + 1}>
                          <div className="grp-head">
                            ...
                            {groupBy === "epic" &&
                              (meta ? <span className={`grp-dot ${meta.c}`} /> : null)}
                            {groupBy === "sprint" &&
                              sprint &&
                              (sprint.st === "active" ? (
                                ...
                                <span className="status ok">
                                  <span className="d"></span>Completed
                                </span>
                              ))}
                            <b>{g.label}</b>
                            {groupBy === "sprint" && sprint && (
                              <span className="tiny muted mono">
                                {sprint.start} – {sprint.end}
                              </span>
                            )}
                            <span className="grp-cnt">
                              {g.items.length} · {ptsTotal(g.items)}pts
                            </span>
                            {((groupBy === "sprint" && sprint) ||
                              (groupBy === "epic" && g.key !== "none")) && (
                              <>
                                <span className="grp-prog">
                                  <i style={{ width: `${prog}%` }} />
                                </span>
                            ...
                            {groupBy === "epic" && g.key !== "none" && (
                              <button className="link grp-open" onClick={() => onOpen(g.key)}>
                                Open epic
                              </button>
                            )}
                          </div>
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    {vis.map((c) => (
                                      ...
                                      >
                                        {c.k === "summary" ? (
                                          <div className="sum-wrap sub-tree">
        ...
        <div className="bl-sec">
          <div className="section-title">
            ...
            <span className="muted tiny">
              {filtered.length} issues · {ptsTotal(filtered)}pts
            </span>
          </div>
          {filtered.length === 0 ? (
            <EmptyIssue
  ...
  return (
    <div className="bl-ep">
      <div className="bl-ep-h" onClick={() => onOpen(String(eid))}>
        ...
        <b>{e.t}</b>
        <span className="grp-cnt">
          {childTasks.length} issues · {ptsTotal(childTasks)}pts
        </span>
        <span className="grp-prog">
          <i style={{ width: `${prog}%` }} />
        </span>
  ...
  return (
    <div className="bl-ep">
      <div className="bl-ep-h">
        ...
        <span className="grp-dot m" />
        <b>No epic</b>
        <span className="grp-cnt">{list.length} issues</span>
      </div>
      {!collapsed && (
        <div className="bl-ep-body">
          <ul className="bl-lvl">
